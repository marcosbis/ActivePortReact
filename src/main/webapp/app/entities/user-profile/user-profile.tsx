...
export const UserProfile = (props: IUserProfileProps) => {
  ...
  const { userProfileList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="user-profile-heading">
        <Translate contentKey="activePortApp.userProfile.home.title">User Profiles</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="activePortApp.userProfile.home.createLabel">Create new User Profile</Translate>
        </Link>
      </h2>
        ...
        {userProfileList && userProfileList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="activePortApp.userProfile.name">Name</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('description')}>
                  <Translate contentKey="activePortApp.userProfile.description">Description</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="activePortApp.userProfile.userRole">User Role</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {userProfileList.map((userProfile, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {userProfile.userRoleName ? <Link to={`user-role/${userProfile.userRoleId}`}>{userProfile.userRoleName}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="activePortApp.userProfile.home.notFound">No User Profiles found</Translate>
            </div>
          )
        )}
      </div>
